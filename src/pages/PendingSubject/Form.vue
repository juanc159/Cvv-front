...
definePage({
  path: "pendingSubject-form/:action/:id?",
  name: "PendingSubject-Form",
  meta: {
...
const fetchDataForm = async () => {
  ...
  const url = form.value.id ? `/pendingSubject/${form.value.id}/edit` : `/pendingSubject/create`
...
const submitForm = async () => {
  ...
  if (validation?.valid) {
    ...
    const url = form.value.id ? `/pendingSubject/update/${form.value.id}` : `/pendingSubject/store`
    ...
    if (response.value?.ok && data.value) {
      ...
      if (data.value.code == 200) {
        router.push({ name: 'PendingSubject-List' })
      }
    }
