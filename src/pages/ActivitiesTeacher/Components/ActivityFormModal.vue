...
const fetchDataForm = async () => {
  ...
  form.user_id = authenticationStore.user?.id || null;
...
const submitForm = async () => {
  ...
  if (!form.company_id || !form.user_id) {
    toast('Error de autenticación', '', 'danger');
    return;
  }

  if (!form.title) {
    toast('El título es requerido', '', 'danger');
    return;
  }
  ...
  try {
    ...
    if (response.status === 200 && data) {
      if (data.code === 200) {
        toast(
          form.id ? 'Actividad actualizada correctamente' : 'Actividad creada correctamente',
          '',
          'success'
        );
        router.push({ name: 'ActivitiesTeacher' });
...
// -------------------- WATCHERS --------------------
watch(() => form.grade_id, () => applyGradeFilter());
watch(() => form.section_id, () => applySectionFilter());
          ...
          <VRow>
            <VCol cols="12">
              <AppTextField label="Título de la Actividad" v-model="form.title" placeholder="Ej: Análisis Literario"
                variant="outlined" density="comfortable" :rules="[requiredValidator]" :disabled="disabledFiledsView" />
            </VCol>

            <!-- Año -->
            <VCol cols="6" sm="4">
              <AppSelect label="Año" :items="selectOptions.grades" item-title="label" item-value="value"
                v-model="form.grade_id" variant="outlined" density="comfortable" clearable
                :disabled="disabledFiledsView || loading.form" />
            </VCol>

            <!-- Sección (depende del año) -->
            <VCol cols="6" sm="4">
              <AppSelect label="Sección" :items="filteredSections" item-title="label" item-value="value"
                v-model="form.section_id" variant="outlined" density="comfortable" clearable
                :disabled="!form.grade_id || disabledFiledsView || loading.form" />
            </VCol>

            <!-- Materia (depende de año+sección) -->
            <VCol cols="12" sm="4">
              <AppSelect label="Materia" :items="filteredSubjects" item-title="label" item-value="value"
                v-model="form.subject_id" variant="outlined" density="comfortable" clearable
            ...
            <VCol cols="12" sm="6">
              <AppTextField type="datetime-local" label="Fecha Límite" v-model="form.deadline_at" variant="outlined"
                density="comfortable" :disabled="disabledFiledsView" />
            </VCol>

            <VCol cols="12" sm="6">
              <AppSelect label="Estado" v-model="form.status" :items="activityStatusEnum_arrayInfo" variant="outlined"
                density="comfortable" :disabled="disabledFiledsView || loading.form" />
            </VCol>
          </VRow>
        </VForm>
      </VCardText>
