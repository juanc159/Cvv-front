...
const crudBannerStore = useCrudBannerStore();
const authenticationStore = useAuthenticationStore();
const router = useRouter();

const { currentPage, totalPage, lastPage, totalData, banners, loading } =
  storeToRefs(crudBannerStore);

// menu data paginate
const rowPerPage = ref<number>(10);

const fetchDataTable = async (data: Array<object> = []) => {
  await crudBannerStore.fetchAll({
...
const paginationData = computed(() => {
  const firstIndex = banners.value.length
    ? (currentPage.value - 1) * totalPage.value + 1
    : 0;
  const lastIndex =
    banners.value.length + (currentPage.value - 1) * totalPage.value;

  return `Mostrando ${firstIndex} a ${lastIndex} de ${totalData.value} registros`;
});

const changeScreen = async (action: string = "create", id?: number) => {
  router.push({ name: "Banner-Form", params: { action: action, id: id } });
};

// Accion cambio de estado
const changeState = async (obj: any, field: string) => {
  const state = !obj[field];
  const promise = await crudBannerStore.changeState({
    id: obj.id,
    value: state,
    field: field,
  });
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await crudBannerStore.fetchDelete(id);
      await fetchDataTable();
...
<template>
  <div>
    <VCard>
      <VCardTitle primary-title>Listado de banners</VCardTitle>
      <VCardText>
        <VDataTable :headers="headers" :items="banners" :items-per-page="rowPerPage">
          <template #top>
            <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
              ...
              <VSpacer />
              <div class="app-banner-search-filter d-flex align-center flex-wrap gap-4">
